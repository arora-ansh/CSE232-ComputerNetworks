...
/*
  Message format to be sent by a sender to receiver
  from: 		Sender's ip
...
class Node {
 private:
  string name; // Label for node
  vector<pair<NetInterface, Node*> > interfaces; // Net interfaces of this node, vector w first = NetInterface, second = Node*
 protected:
 ...
 public:
  ...
  void printTable() {
    Comparator myobject;
    sort(mytbl.tbl.begin(),mytbl.tbl.end(),myobject); // Sort the table based on cost
  ...
  void sendMsg(){
    struct routingtbl ntbl; // This is a new table for <something>
    for (int i = 0; i < mytbl.tbl.size(); ++i) {
      ntbl.tbl.push_back(mytbl.tbl[i]); // Copying the contents of mytbl to ntbl
    }
    // Traversing through all the interfaces of this node
    for (int i = 0; i < interfaces.size(); ++i) {
      RouteMsg msg; // Message to be sent
      msg.from = interfaces[i].first.getip(); // Sender's ip, starting point 
      msg.mytbl = &ntbl; // We attach the table deep copied to the message being sent
      msg.recvip = interfaces[i].first.getConnectedIp(); // Connected IP info being sent to interface's second held node	
      interfaces[i].second->recvMsg(&msg); // We will make the node pointed by second of interface recv the Msg 
    }
  }
...
class RoutingNode: public Node {
 public:
  void recvMsg(RouteMsg *msg); // Recv msg defined in routing_algo.cpp
};
