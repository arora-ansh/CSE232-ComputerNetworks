...
int main() {
    ...
    distanceVectorNodes.clear(); // Clear the vector storing the nodes
    for (int i = 0 ; i < n; i++) {
      RoutingNode *newnode = new RoutingNode(); // Routing node is created here, declared in node.h
      cin>>name;
      newnode->setName(name); // Name is set as the label of the node (A, B, C etc)
